...
const tipoduracaoInput = document.getElementById('tipoDuracao');
...
if (form) {
  form.addEventListener('submit', async (e) => {
    ...
    const novo = {
      ...
      tipo_duracao: tipoduracaoInput.value,
...
function openModal() {
  if (editingIndex > -1) {
    ...
    horaFimInput.value = s.hora_fim || '';
    telefoneInput.value = s.telefone || '';
    duracaoInput.value = s.duracao || 1;
    tipoduracaoInput.value = s.tipo_duracao || 'dias';
...
function renderList() {
  ...
  filtrados.forEach((s, index) => {
    ...
    const fim = calcularFimServico(s.data, s.duracao, s.tipo_duracao);
    const fidelizadoBadge = s.fidelizado ? '<span class="badge">⭐ Fidelizado</span>' : '';
    const statusBadge = s.status === 'concluido' ? '<span class="badge status-badge">✅ Concluído</span>' : '<span class="badge status-badge pendente">⏳ Pendente</span>';
    ...
    const finalizarBtn = s.status === 'pendente' ? `<button class="small-btn finalizar" onclick="finalizarServico(${realIndex})">✅ Finalizar</button>` : '';
    const duracaoText = s.tipo_duracao === 'horas' ? `${s.duracao}h` : `${s.duracao} dia(s)`;

    card.innerHTML = `
      <div class="title">${s.empresa || '—'}</div>
      <div class="meta">${s.categoria || '—'} • ${formatDateDisplay(s.data)} ${s.hora ? '• ' + s.hora : ''}</div>
      <div class="info">
        <strong>Início:</strong> ${formatDateDisplay(s.data)} ${s.hora ? s.hora : ''} | 
        <strong>Fim previsto:</strong> ${formatDateDisplay(fim)} ${s.hora_fim ? s.hora_fim : ''} | 
        <strong>Duração:</strong> ${duracaoText}
      </div>
...
async function finalizarServico(index) {
  ...
  try {
    ...
    if (error) {
      ...
      showSaveNotification('✓ Serviço finalizado com sucesso!');
      await carregarAgendamentos();
      renderList();
      renderCalendar();
    }
...
function calcularFimServico(dataInicial, duracao, tipoDuracao) {
  if (!dataInicial) return '';
  
  if (tipoDuracao === 'horas') {
    // Se é em horas, retorna a mesma data
    return dataInicial;
  } else {
    // Se é em dias, calcula a data final
...
function renderCalendar() {
  ...
  for (let d = 1; d <= totalDias; d++) {
    ...
    servicos.forEach(s => {
      ...
      if (s.tipo_duracao === 'horas') {
        // Se é em horas, só marca o dia do agendamento
        if (formatDateISO(start) === dataStr) {
          servNoDia.push(s);
        }
      } else {
        // Se é em dias, marca todos os dias da duração
...
function openDayModal(dataISO) {
  ...
  encontrados.forEach(s => {
    ...
    if (s.status === 'concluido') {
      item.classList.add('status-concluido');
    }

    const fim = calcularFimServico(s.data, s.duracao, s.tipo_duracao);
    const fidelizadoMark = s.fidelizado ? ' ⭐ (Fidelizado)' : '';
    const statusMark = s.status === 'concluido' ? ' ✅ (Concluído)' : '';
    const whatsappBtn = s.telefone ? `<button class="small-btn whatsapp" onclick="enviarWhatsApp('${s.telefone}', '${s.empresa}', '${formatDateDisplay(s.data)}', '${s.hora || ''}')">📱 WhatsApp</button>` : '';
    const finalizarBtn = s.status === 'pendente' ? `<button class="small-btn finalizar" onclick="finalizarServico(${realIndex})">✅ Finalizar</button>` : '';
    const duracaoText = s.tipo_duracao === 'horas' ? `${s.duracao}h` : `${s.duracao} dia(s)`;

    item.innerHTML = `
      <div>${s.empresa}${fidelizadoMark}${statusMark}</div>
      <div class="small-meta">
        <strong>${s.categoria}</strong> • Início: ${formatDateDisplay(s.data)} ${s.hora || ''} • Fim: ${formatDateDisplay(fim)} ${s.hora_fim || ''} • ${duracaoText}
      </div>
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .concluido { background: #d4edda; padding: 2px 8px; border-radius: 4px; font-size: 12px; font-weight: 600; color: #155724; }
...
`;
  ...
  servicos.forEach(s => {
    ...
    const statusBadge = s.status === 'concluido' ? '<span class="concluido">✅ Concluído</span>' : '⏳ Pendente';
    const obs = s.obs || '—';
    const duracaoText = s.tipo_duracao === 'horas' ? `${s.duracao}h` : `${s.duracao} dia(s)`;
